...
mBOOL DLLINTERNAL MPlugin::ini_parseline(const char *line) {
	...
	if(unlikely(!tmp_line))
		RETURN_ERRNO(mFALSE, ME_NOMEM);

	// skip whitespace at start of line
	while(unlikely(*tmp_line==' ') || unlikely(*tmp_line=='\t')) tmp_line++;

	// remove whitespace at end of line
	cp = tmp_line + strlen(tmp_line) -1;
	while(unlikely(*cp==' ') || unlikely(*cp=='\t')) *cp--='\0';

	// skip empty lines
	if(unlikely(tmp_line[0]=='\0')) {
		META_DEBUG(7, ("ini: Ignoring empty line: %s", tmp_line));
		free(tmp_line);
		RETURN_ERRNO(mFALSE, ME_BLANK);
	}

	if(unlikely(tmp_line[0]=='#') || unlikely(tmp_line[0]==';') || unlikely(strstr(tmp_line, "//")==tmp_line)) {
	...
	if(likely(strcasecmp(token, PLATFORM)) == 0) {
		pfspecific=0;
	} else if(likely(strcasecmp(token, PLATFORM_SPC)) == 0) {
		pfspecific=1;
	} else {
...
mBOOL DLLINTERNAL MPlugin::check_input(void) {
	...
	if(unlikely(!file) || unlikely(!file[0])) {
...
mBOOL DLLINTERNAL MPlugin::resolve(void) {
	...
	if(likely(is_absolute_path(filename)))
	...
	if(likely(strncasecmp(pathname, GameDLL.gamedir, len) == 0))
...
char * DLLINTERNAL MPlugin::resolve_prefix(const char *path) {
	...
	if(unlikely((found=resolve_suffix(buf))))
		return(found);

	// try other suffixes for the original path
	if(unlikely((found=resolve_suffix(path))))
		return(found);
...
mBOOL DLLINTERNAL MPlugin::platform_match(MPlugin* other) {
	...
	if(unlikely(end == NULL) || unlikely(!is_platform_postfix(end))) 
		end=strrchr(file, '.');
	
	other_end=strrchr(other->file, '_');
	if(unlikely(other_end == NULL) || unlikely(!is_platform_postfix(other_end)))
		other_end=strrchr(other->file, '.');
	
	if(unlikely(end == NULL) || unlikely(other_end == NULL))
		return(mFALSE);
...
mBOOL DLLINTERNAL MPlugin::load(PLUG_LOADTIME now) {
	...
	if(unlikely(action != PA_LOAD) && unlikely(action != PA_ATTACH) && unlikely(action != PA_RELOAD)) {
	...
	if(likely(status <= PL_OPENED)) {
		...
		if(unlikely(!query())) {
			...
			if(likely(meta_errno != ME_DLOPEN)) {
	...
	if(unlikely(info->loadable < now)) {
		if(likely(info->loadable > PT_STARTUP)) {
	...
	if(likely(now != PT_STARTUP)) {
		FN_GAMEINIT pfn_gameinit=NULL;
		if(likely(tables.dllapi) && unlikely((pfn_gameinit=tables.dllapi->pfnGameInit)))
			pfn_gameinit();
	}
...
mBOOL DLLINTERNAL MPlugin::query(void) {
	...
	if(unlikely(!(pfn_give_engfuncs = (GIVE_ENGINE_FUNCTIONS_FN) DLSYM(handle, "GiveFnptrsToDll")))) {
	...
	if(unlikely(pfn_query(META_INTERFACE_VERSION, &info, &mutil_funcs) != TRUE)) {
	...
	// error.
	if(likely(info) && unlikely(!FStrEq(info->ifvers, META_INTERFACE_VERSION))) {
		...
		// (update metamod).
		if(unlikely(pmajor > mmajor) || (likely(pmajor==mmajor) && unlikely(pminor > mminor))) {
		...
		// (update plugin).
		else if(unlikely(pmajor < mmajor)) {
		...
		else if(likely(pmajor==mmajor) && likely(pminor < mminor))
	...
	if(unlikely(meta_errno == ME_IFVERSION)) {
	...
	else if(unlikely(meta_errno != ME_NOERROR)) {
...
#define GET_FUNC_TABLE_FROM_PLUGIN(pfnGetFuncs, STR_GetFuncs, struct_field, API_TYPE, TABLE_TYPE, vers_pass, vers_int, vers_want) \
	if(unlikely(meta_table.pfnGetFuncs)) { \
		if(likely(!struct_field)) { \
			struct_field = (TABLE_TYPE*)calloc(1, sizeof(TABLE_TYPE)); \
		} else { \
			memset(struct_field, 0, sizeof(TABLE_TYPE)); \
		} \
		if(likely(meta_table.pfnGetFuncs(struct_field, vers_pass))) { \
	...
	if(unlikely(!tables.dllapi) && likely(!post_tables.dllapi)) {
	...
	if(unlikely(!tables.dllapi) && likely(!post_tables.dllapi)
		&& likely(!tables.newapi) && likely(!post_tables.newapi)
		&& likely(!tables.engine) && likely(!post_tables.engine))
	{
...
mBOOL DLLINTERNAL MPlugin::plugin_unload(plid_t plid, PLUG_LOADTIME now, PL_UNLOAD_REASON reason) {
	...
	// try find unloader
	if(unlikely(!(pl_unloader=Plugins->find(plid)))) {
	...
	else if(unlikely(pl_unloader->index == index)) {
	...
	if(unlikely(meta_errno==ME_DELAYED)) {
		action=old_action;
...
mBOOL DLLINTERNAL MPlugin::unload(PLUG_LOADTIME now, PL_UNLOAD_REASON reason, PL_UNLOAD_REASON real_reason) {
	...
	if(likely(info) && unlikely(info->unloadable < now)) {
		if(unlikely(reason == PNL_CMD_FORCED)) {
			...
			if(likely(info->unloadable > PT_STARTUP)) {
	...
	if(likely(action==PA_UNLOAD)) {
		status=PL_EMPTY;
		clear();
	}
	else if(likely(action==PA_RELOAD)) {
		status=PL_VALID;
		action=PA_LOAD;
		clear();
	}
...
mBOOL DLLINTERNAL MPlugin::reload(PLUG_LOADTIME now, PL_UNLOAD_REASON reason) {
	...
	if(likely(info) && unlikely(info->loadable < now)) {
		if(likely(info->loadable > PT_STARTUP)) {
...
mBOOL MPlugin::pause(void) {
	...
	if(unlikely(info->unloadable < PT_ANYPAUSE)) {
...
mBOOL DLLINTERNAL MPlugin::retry(PLUG_LOADTIME now, PL_UNLOAD_REASON reason) {
	if(unlikely(action==PA_LOAD))
		...
		return(load(now));
	else if(unlikely(action==PA_UNLOAD))
		return(unload(now, reason, reason));
	else if(unlikely(action==PA_RELOAD))
...
void DLLINTERNAL MPlugin::free_api_pointers(void) {
	...
	if(likely(tables.dllapi))
		free(tables.dllapi);
	if(unlikely(post_tables.dllapi))
		free(post_tables.dllapi);
	if(unlikely(tables.newapi))
		free(tables.newapi);
	if(unlikely(post_tables.newapi))
		free(post_tables.newapi);
	if(unlikely(tables.engine))
		free(tables.engine);
	if(unlikely(post_tables.engine))
...
mBOOL DLLINTERNAL MPlugin::clear(void) {
	...
	if(likely(handle) && unlikely(DLCLOSE(handle) != 0)) {
...
void DLLINTERNAL MPlugin::show(void) {
	...
	if(unlikely(Plugins->found_child_plugins(index)))
		Plugins->show(index);
	else
...
mBOOL DLLINTERNAL MPlugin::newer_file(void) {
	...
	if(unlikely(stat(pathname, &st) != 0))
	...
	if(unlikely(file_time > time_loaded))
		return(mTRUE);
	else
...
const char * DLLINTERNAL MPlugin::str_status(STR_STATUS fmt) {
	switch(status) {
		...
		case PL_VALID:
			if(likely(fmt==ST_SHOW)) return("info");
			else return("valid");
		case PL_BADFILE:
			if(likely(fmt==ST_SHOW)) return("badf");
			else return("badfile");
		case PL_OPENED:
			if(likely(fmt==ST_SHOW)) return("open");
			else return("opened");
		case PL_FAILED:
			if(likely(fmt==ST_SHOW)) return("fail");
			else return("failed");
		case PL_RUNNING:
			if(likely(fmt==ST_SHOW)) return("RUN");
			else return("running");
		case PL_PAUSED:
			if(likely(fmt==ST_SHOW)) return("PAUS");
			else return("paused");
		default:
			if(likely(fmt==ST_SHOW)) return(META_UTIL_VarArgs("UNK%d", status));
...
const char * DLLINTERNAL MPlugin::str_action(STR_ACTION fmt) {
	switch(action) {
		...
		case PA_NONE:
			if(likely(fmt==SA_SHOW)) return(" -  ");
			else return("none");
		case PA_KEEP:
			if(likely(fmt==SA_SHOW)) return("keep");
			else return("keep");
		case PA_LOAD:
			if(likely(fmt==SA_SHOW)) return("load");
			else return("load");
		case PA_ATTACH:
			if(likely(fmt==SA_SHOW)) return("atch");
			else return("attach");
		case PA_UNLOAD:
			if(likely(fmt==SA_SHOW)) return("unld");
			else return("unload");
		case PA_RELOAD:
			if(likely(fmt==SA_SHOW)) return("relo");
			else return("reload");
		default:
			if(likely(fmt==SA_SHOW)) return(META_UTIL_VarArgs("UNK%d", action));
...
const char * DLLINTERNAL MPlugin::str_loadtime(PLUG_LOADTIME ptime, STR_LOADTIME fmt) {
	switch(ptime) {
		...
		case PT_STARTUP:
			if(likely(fmt==SL_SHOW)) return("Start");
			else if(likely(fmt==SL_ALLOWED)) return("at server startup");
			else if(likely(fmt==SL_NOW)) return("during server startup");
			else return("startup");
		case PT_CHANGELEVEL:
			if(likely(fmt==SL_SHOW)) return("Chlvl");
			else if(likely(fmt==SL_ALLOWED)) return("at changelevel");
			else if(likely(fmt==SL_NOW)) return("during changelevel");
			else return("changelevel");
		case PT_ANYTIME:
			if(likely(fmt==SL_SHOW)) return("ANY");
			else if(likely(fmt==SL_ALLOWED)) return("at any time");
			else if(likely(fmt==SL_NOW)) return("during map");
			else return("anytime");
		case PT_ANYPAUSE:
			if(likely(fmt==SL_SHOW)) return("Pause");
			else if(likely(fmt==SL_ALLOWED)) return("at any time, and pausable");
			else if(likely(fmt==SL_NOW)) return("for requested pause");
			else return("pausable");
		default:
			if(likely(fmt==SL_SHOW)) return(META_UTIL_VarArgs("UNK-%d", ptime));
...
const char * DLLINTERNAL MPlugin::str_reason(PL_UNLOAD_REASON preason, PL_UNLOAD_REASON preal_reason) {
	...
	if(unlikely(preason == PNL_PLUGIN))
		preason = PNL_NULL;
	if(unlikely(preason == PNL_PLG_FORCED))
		preason = PNL_NULL;
...
const char * DLLINTERNAL MPlugin::str_source(STR_SOURCE fmt) {
	switch(source) {
		...
		case PS_CMD:
			if(likely(fmt==SO_SHOW)) return("cmd");
			else return("console command");
		case PS_PLUGIN:
			if(unlikely(source_plugin_index <= 0)) {
				if(likely(fmt==SO_SHOW)) return("plUN");
				else return("unloaded plugin");
			} else {
				if(likely(fmt==SO_SHOW)) return(META_UTIL_VarArgs("pl%d", source_plugin_index));
				else return(META_UTIL_VarArgs("plugin [%d]", source_plugin_index));
			}
		default:
			if(likely(fmt==SO_SHOW)) return(META_UTIL_VarArgs("UNK%d", source));
